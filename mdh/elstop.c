...
struct layers *layers;
{
  int i,n,j,counts,k,l,jakaja,ii;
   ...
   fprintf(file->fpo,"Read in %d el.stopping points from %s for layer %d , last stopping=%g\n",
	 ...
	 elstop->stopp[i]*=2.34951e-05; /* unit conversion??*/
	 if(i==1) elstop->slim=(elstop->strs[1]-elstop->strs[0]);
         if(i>1) 
           if(100*fabs(elstop->slim-elstop->strs[i]+elstop->strs[i-1])>elstop->slim) elstop->slim=0;
	 /*printf("%lg %lg\n",a,elstop->stopp[i]);*/
   ...
   if(elstop->slim!=0) elstop->iperslim=1.0/elstop->slim;  /*needed in the future, just a speedup */
...
struct file *file;
{
   ...
   static double stragglingsum=0.0,stragglingn=0.0;
...
cha+=ti*ui*vi*elstop->rho[xup][yup][zup]+(1-ti)*ui*vi*elstop->rho[xdown][yup][zup];
 ...
 onelrad=exp(temp);  /*note that rs was in reality log(rs) in atomic units!! */
     ...
     if(elstop->correction)
       if(onelrad>3.0) onelrad*=pow(5.0,-0.33333); /*efective r_s for low el. densities in channels (n<0.0088 a.u. or 0.0597 e/Ong^3)*/
...
onelrad+=ti_ui_vi*apurho[axu+ayu+zup]+(vi_ui-ti_ui_vi)*apurho[axd+ayu+zup];
 ...
 onelrad=exp(onelrad);  /*note that rs was in reality log(rs)!! */
     ...
     if(elstop->correction)
       if(onelrad>3.0) onelrad*=pow(5.0,-0.33333); /*efective r_s for low el. densities in channels (n<0.0088a.u. or 0.0597 e/Ong^3)*/
