...
moveslice(at,atex,atflag,type,iniat,layers,nlayer,nat,box,
	  recoil,reccalc,recresult,update,firstrecstep,recoilspec,physical,unit,file)
struct atom *at;
struct atomex *atex;
...
logical *update,firstrecstep,recoilspec;
{
   ...
   real E,r,Theta,Phi,x,y,z,mx,my,mz;
   point center;
...
/* Step 0: Check whether movement is needed */
   ...
   *update=True;
   ...
   for(l=0;l<3;l++) {  /* Select one direction to move in */
      ...
      MAXDEBUGSR("moveslice: getting amo state",namorph);
      getamorphstate(&namorph,recresult,recoil->sz,box->movelim.z,physical,layers,*nlayer);
      MAXDEBUGSR("moveslice: got amo state",namorph);
      ...
      if(l==2) 
	{
      ...
      MAXDEBUGSR("moveslice: moving layers",layers->Nlayers);
	 ...
	 /*movelimits for fronts at the same place for each box.*/
       ...
       MAXDEBUGSR("moveslice: moved layers",layers->Nlayers);
				   ...
				   zscale<(1.0+tol) && zscale>(1.0-tol)))
	 {
       ...
       MAXDEBUGSRR("moveslice: getting initial coords",nlayernext,namorphnext);
       if (layers->layer[nlayernext].natoms[namorphnext] >= ORIGMAXATOM) {
	 fprintf(stderr,"mdh Error: ORIGMAXATOM %d in arraysizes.h too small, increase to Natoms %d in coords.in and recompile\n\n",ORIGMAXATOM,layers->layer[nlayernext].natoms[namorphnext]);
	 exit(0);
       }
	 ...
	 MAXDEBUGSRR("moveslice: scaling needed",nlayernext,namorphnext);
	for(i=0;i<layers->layer[nlayernext].natoms[namorphnext];i++)
	   {
	   x=iniat[iniind(nlayernext,namorphnext,nini,i)].x;
	   y=iniat[iniind(nlayernext,namorphnext,nini,i)].y;
       ...
       MAXDEBUGSR("moveslice: step 3",recoil->number);
	 ...
	 if (moved) {
	    ...
	    if (recoilspec) {
		  ...
		  Theta=acos(Theta); 
		  Phi=atan2(at[i].vy,at[i].vx);

		  itype=at[i].type;
		  MAXDEBUGSRRR("recspec",i,E,Theta);

		  if (reccalc->collisionsout>=0.0 && E>reccalc->collisionsout) {
		    fprintf(file->fpcollisions,"x %.10g %.10g %.10g v %g %g %g t %d th %g ph %g E %g\n",
			    box->movement.x+at[i].x,box->movement.y+at[i].y,box->movement.z+at[i].z,unit->vel[itype]*at[i].vx,unit->vel[itype]*at[i].vy,unit->vel[itype]*at[i].vz,itype,Theta,Phi,E);
		  }
      ...
      /*printf("moved %d out of %d atom in old box\n",j,*nat);*/
	 ...
	 for (i=0;i<ii;i++)
	   {
	     ...
	     if (layers->layer[nlayernext].shiftmode>0 && moved) {
	       /* Do a random shift of atom coordinates in xy to prevent channeling */
	       if (move.z!=0) {
		 if (layers->layer[nlayernext].shiftmode==1) {
		   tempat[i].x+=unirand(0)*box->size.x; if (tempat[i].x > box->size.x) tempat[i].x-=box->size.x;
		   tempat[i].y+=unirand(0)*box->size.y; if (tempat[i].y > box->size.y) tempat[i].y-=box->size.y;
		 }
		 else if (layers->layer[nlayernext].shiftmode==2) {
		   tempat[i].x+=(int)(unirand(0)*box->size.x/box->movelim.x)*box->movelim.x; if (tempat[i].x > box->size.x) tempat[i].x-=box->size.x;
		   tempat[i].y+=(int)(unirand(0)*box->size.y/box->movelim.y)*box->movelim.y; if (tempat[i].y > box->size.y) tempat[i].y-=box->size.y;
		 }
	       }
	    }
