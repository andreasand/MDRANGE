...
/* Print out recoil energy and velocity */
   ...
   /*  check for implantations in Si with PENR JARKKO*/
   if(elstop->penr)
   {
...
/* Calculate the depen table step using a very simple method */
   ...
   recresult->sumFDsput=0.0; recresult->sumFDtrans=0.0; 
...
/* Initialize stopping table variables */
   ...
   if (file->printavvel) {
      ...
      */
      ...
      /* KAI: This was broken at some point of elstop development */
      ...
      */
     /* Simple replacement that likely works for pretty much all ions */
     recresult->avveltmax=100000.0/unit->time[recoil->attype]*1e-15; 
     recresult->avveldt=recresult->avveltmax/AVVELSIZE;
...
*/
   if (reccalc->poly && poly->ndomain != 0 && reccalc->Polydeltaangle > 0.0) {
	 ...
	 /* Disabled this weird feature for V3.06 on 28.3.2016: confusing for channeling sims */
...
*/
   ...
   /* Changed for V3.07 */
   recoil->r_proj=(recoil->vx0*(recoil->sx-recoil->x0)
		   +recoil->vy0*(recoil->sy-recoil->y0)
		   +recoil->vz0*(recoil->sz-recoil->z0))/recoil->vel0;
   recoil->r_proj=recoil->r_proj+recoil->z0/cos(recoil->theta);
...
struct poly *poly;
{
   ...
   /* This had a small error until 28.3.2016: projection was not
      normalized to starting point... Could cause artefacts when using 
      large theta and projected range (Trange=1): too many "sputtered" 
      atoms as an artefact */
   recoil->r_proj=(recoil->vx0*(recoil->sx-recoil->x0)
		   +recoil->vy0*(recoil->sy-recoil->y0)
		   +recoil->vz0*(recoil->sz-recoil->z0))/recoil->vel0;
   /* V3.07 Correction factor to account for distance to surface (z=0) from initial
      point. This will fail if  recoil->theta=90 but is forbidden in readin now*/
   recoil->r_proj=recoil->r_proj+recoil->z0/cos(recoil->theta);

   MAXDEBUGSRRR("getrecprop sx:",recoil->sx,recoil->sy,recoil->sz);
   ...
   MAXDEBUGSRRRRR("getrecprop:",r,recoil->vel,recoil->acc,recoil->dr,recoil->sz);
...
int nat;
{
   ...
   maxima->vel=sqrtf(maxima->vel); 
   maxima->acc=sqrtf(maxima->acc); 
...
calcmisc(recoil,elstop,reccalc,recresult,poly,file,firstrecstep,t,physical)
/*
...
real t;
{
   ...
   FDe=elstop->Se*recoil->dr;
   FDf=elstop->totalfirsov*recoil->dr; /*JARKKO*/
   FDn=FD-FDe-FDf; /*JARKKO*/
	 ...
	 if (dr != 0.0) {
	   S=(recoil->eprev-recoil->energy)/dr;/**(recoil->vprev-recoil->vel);*/
	    ...
	    if(recresult->stopR>reccalc->stopDR)
	      {
	      i=(int) (recresult->stopv/recresult->istop) /recresult->stopdv;
...
/*
   ...
   stopway = 0:  Energy below threshold reccalc->Emin 
	   ...
	   = 8:  Recoil became bound (only possible for attractive potentials and antiprotons)

   Valid stopways for the range being stored in recresult are 0 and 4.
   For avvel calculations, all stopways are valid.
...
*/
   ...
   /* This clause was inconsistent with main loop, changed to same as there in V3.07 */
   if (issput) stopway=1;
   ...
   if (recbound) stopway=8;

   if (file->printrecend) {
     fprintf(file->fpo,"Recoil end pos %.2f %.2f %.2f E %.8g r %g v %g %g %g \n",
	     recoil->sx,recoil->sy,recoil->sz,recoil->energy,recoil->range,
	     recoil->vx,recoil->vy,recoil->vz);
   }
   ...
   if (stopway == 0 || stopway == 4) {
	...
	recresult->ranges[i]=table[i];
      ...
      recresult->ranges[recresult->nrange]=recoil->range- physical->sput_surf; /*JARKKO erosion,range mitataan nollasta*/
      recresult->nrange++;
   ...
   /* Changed in mdrange V3.08 to always do avvel calculation */
   /* if (stopway == 0 || stopway == 4 || stopway==7) { */
      recresult->nvalidavvel++;
      if (file->printavvel) {
	 for(i=0;i<AVVELSIZE;i++) {
	 ...
	 MAXDEBUGSRRR("avvel collect",0,recresult->avvel[0],recresult->nvalidavvel);
      }
      /* }     */ 


   if (stopway==1) recresult->sumFDsput+=recoil->energy;
...
int nrec;
{
   ...
   int rarray[RARRAYSIZE];
   double Snsum,Sesum;
   double avvelsum;
   char buf[120];
   real a,b,area; /*JARKKO*/
   ...
   mean=0.0; sigmar=0.0; straggle=0.0;
   for(i=0;i<recresult->nrange;i++) {
   ...
   if (recresult->nrange>0) mean=sum/recresult->nrange;
   ...
   jmax=1;
   for (i=0;i<recresult->nrange;i++) {      
      dmean+=(recresult->ranges[i]-mean)*(recresult->ranges[i]-mean);
   ...
   if (recresult->nrange>1) {  
     straggle=sqrtf(dmean/recresult->nrange);
     sigmar=sqrtf(dmean/recresult->nrange/(recresult->nrange-1));
   }

   fprintf(file->fpo,"Ways of stopping:");
   for (i=0;i<=8;i++) fprintf(file->fpo," %d",recresult->stopway[i]);
   fprintf(file->fpo," (0 stop, 1 sput, 7 trans)\n");
   ...
   for (j=0;j<=jmax;j++) fprintf(file->fpra,"%g %d\n",minr+j*deltar,rarray[j]);
   fclose(file->fpra);
   ...
   if (file->printdepen) {
      fprintf(file->fpo,"Total deposited energies/ion: FDn %g FDe %g FDf %g for nrec %d ions\n",
	      recresult->sumFDn/nrec,recresult->sumFDe/nrec,recresult->sumFDf/nrec,nrec);
   ...
   if (file->recoilspec) {
	 ...
	 else sprintf(buf,"%s.out",file->recspecf);
	 ...
	 else sprintf(buf,"%s.out",file->recspec_Thetaf);
	 file->fprst=fopen(buf,"w");
	 fprintf(file->fprst,"Lowest energy: %g, Highest energy: %g \n",reccalc->ETmin,reccalc->ETmax);
	 for(j=0;j<=i+1;j++) 
	    {
   ...
   if (file->printavvel) {
      DEBUGS("Writing average velocity file avvel\n");
      avvelsum=0.0;
      file->fpav=fopen(file->avvelf,"w");
      for(i=0; i<AVVELSIZE;i++) {
	 if (recresult->iavvel[i] !=0 && recresult->nvalidavvel>0) {
	    fprintf(file->fpav,"%g %g %d\n",
		    (i+0.5)*recresult->avveldt*unit->time[reccalc->Atype]/1e-15,
		    recresult->avvel[i]/recresult->nvalidavvel*unit->vel[reccalc->Atype],
		    recresult->iavvel[i]*recresult->nvalidavvel);
	    avvelsum+=recresult->avvel[i]/recresult->nvalidavvel*unit->vel[reccalc->Atype]*recresult->avveldt*unit->time[reccalc->Atype]/1e-15;
	 }
      }
      fclose(file->fpav);

      fprintf(file->fpo,"Integrated average velocity %g for %d valid avvel trajectories\n",avvelsum,recresult->nvalidavvel);
   }
   fprintf(file->fpo,"\n");
