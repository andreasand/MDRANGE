...
*/
   ...
   if (firstpair[ti][tj]) {
	 ...
	 if (n>=2) {
      ...
      N=i;
      fprintf(file->fpo,"reppot() Read in %d valid-seeming repulsive potential points\n",N);
      fclose(file->fpre);
   ...
   *V=a*VR[klo][ti][tj]+b*VR[khi][ti][tj]+((atemp*a-a)*VR2[klo][ti][tj]+(btemp*b-b)*VR2[khi][ti][tj])*htemp;
   ...
   /* This used to have an error in mdh3, + in front of 3*btemp ! */
   /* Corrected in version 3.02. */
