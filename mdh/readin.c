...
struct reccalc *reccalc;
{
   ...
   for (i=0;i<MAXTYPE;i++) {
      ...
      /*if(readstat) file->layeratoms[i]=1;*/
      /*printf("Found atom ? %d %d\n",i,file->layeratoms[i]);*/
...
struct layers *layers;
{
   ...
   /*JARKKO-damage*/
   /*RIC(layers->Namorphmax,1,"Maximum number of amorphization states in one layer");*/
   ...
   for (i=0; i< layers->Nlayers && i<MAXLAYER;i++) {
      ...
      RICA(layers->layer[i].Namorph,i,1, "Number of amorphous states, 1=no am. states"); /*JARKKO*/
      RICA(layers->layer[i].elstop,i,0,     "0=ZBL,1=Puska,2=BK elstop "); /*JARKKO*/
      RICA(layers->layer[i].fulldensity,i,0,     "0=off=default,1=using full charge density file"); /*JARKKO*/
      RRCA(layers->layer[i].surfbind,i,4.63,     "surface binding energy (eV)"); /*JARKKO*/
      RICA(layers->layer[i].shiftmode,i,0,     "0=off=default,1=do cell shift in x y"); /*JARKKO*/
      if(max<layers->layer[i].Namorph) max=layers->layer[i].Namorph;  /*JARKKO*/
      if (layers->layer[i].Probability>1.0) 
	 fprintf(fpdebug,"readlayers: Warning !! : layer probability > 1\n");
      fprintf(file->fps,"\n");
   }
   MAXAMORPH=max;  /* Taken from biggest quantitity */
   DEBUG(MAXAMORPH);
   oflush();
}
...
struct box *box;
{
   ...
   RIC(box->nomoveslice,0,"nomoveslice flag (1: disable moveslice)");
}
/*--------------------------------------------------------------------------*/
readreccalc(file,reccalc,box)
struct file *file;
...
struct box *box;
{
   ...
   RIC(reccalc->allowhugeenergy,0,"Allow huge energies (e.g. for antiprotons)");
   ...
   if (reccalc->Theta0 >=90.0 || reccalc->Thetamax>=90.0) {
     fprintf(file->fpdb,"readreccalc ERROR: Theta>=90 degrees not possible %g %g\n",reccalc->Theta0,reccalc->Thetamax);
     exit(0);
   }
   ...
   RRC(reccalc->stopDR,4.0,"Thickness of slices for Stopping calcs");
...
struct box *box;
{
   ...
   RRC(potcrit->rfirsov,1.47,"Muffin-tin radius");
...
struct physical *physical;
{
   ...
   /*JARKKO-damage*/
   RIC(physical->Damage,0,"Damage buildup flag (0-inactive)");
   RRC(physical->Dose,1e11,"Dose");
   RIC(physical->scaling,0,"scaling between different boxes/layers");
   RIC(physical->bscaling,0,"scaling in the initstate for coords.num");
   RIC(physical->sputtering,0,"sputtering flag (0-inactive)");
   RIC(physical->spreadFD,0,"if FDn needs spreading in damage model- flag (0-inactive)");
   RRC(physical->sput_surf,0,"amount of erosion, used in sputtering for example");
   RRC(physical->Y,0.0,"Fixed value for sput. yield, if needed");
   RRC(physical->sputFD,0.0,"Fixed value for surf. depon. nucl. energy, if needed");
   RRC(physical->sputdeltaV,0.0,"Value controlling the volume increase in sputtering model");
}
/*--------------------------------------------------------------------------*/
readgen(file,gen)
struct file *file;
...
struct file *file;
{
   ...
   RIC(file->movie,0,"movie?, 1=yes,0=no"); 
...
struct reccalc *reccalc;
{
   ...
   RIC(elstop->Firsov,0,"Use Firsov model?");
   RIC(elstop->correction,0,"Scaled el. density in channels, may improve <110> results with 3d-ch. density.");
   ...
   RRC(elstop->dx,5.43/128,"dx in rho data");
   RRC(elstop->dy,5.43/128,"dy in rho data");
   RRC(elstop->dz,5.43/128,"dz in rho data");
   RRC(elstop->ux,5.43,"unit cell, x, rho-data");
   RRC(elstop->uy,5.43,"unit cell, y, rho-data");
   RRC(elstop->uz,5.43,"unit cell, z, rho-data");
