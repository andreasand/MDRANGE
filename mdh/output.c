...
real t;
{
   ...
   for (i=0;i<nat;i++) printf("co %g %g %g %.10g %d\n",at[i].x,at[i].y,at[i].z,t,i);
   for (i=0;i<nat;i++) printf("ve %g %g %g %.10g %d\n",at[i].vx,at[i].vy,at[i].vz,t,i);
   for (i=0;i<nat;i++) printf("ac %g %g %g %.10g %d\n",at[i].ax,at[i].ay,at[i].az,t,i);
}   
...
int n;
{
   ...
   for (i=0;i<nat;i++) printf("co %.9g %.9g %.9g %.10g %d\n",
			      box->movement.x+at[i].x,
			      box->movement.y+at[i].y,
			      box->movement.z+at[i].z,t,i);
}   
