...
int splitlimitcheck(recoil,recresult)
struct recoil *recoil;
struct recresult *recresult;
{
  int a=0;
  if(recoil->splitnum<MAXSPLITBORDERS &&
     recoil->sz > recresult->splitborders[recoil->splitnum]) 
      a=1;
  return(a);
}

splitlimitcalc(recresult)
struct recresult *recresult;
{
  int i,j,border,jmax;
  real sorted[RARRAYSIZE];
  real sum,nsum;
  
  if(recresult->nrange>40) /*first some statistics to the range profile..*/
    {
      sum=0.0;jmax=0;
      for(i=0;i<RARRAYSIZE; i++) sorted[i]=0;
  for(i=0; i<recresult->nrange; i++)
    {
      /*sum+=recresult->ranges[i]*recresult->weights[i];*/
    j=(int)floor(recresult->ranges[i]);
    if(j<RARRAYSIZE && j>=0) 
      {sorted[j]+=recresult->weights[i];
      sum+=recresult->weights[i];
      if(j>jmax) jmax=j;
      }
    }
  ...
  for(i=0; i<jmax; i++)
    ...
    {
    nsum+=sorted[i];
    if(nsum>=(1.0-pow(0.5,border+1))*sum)
     if(border<MAXSPLITBORDERS)
      {
      recresult->splitborders[border]=i;
      border++;
      }  
    }
  /*for (i=0;i<MAXSPLITBORDERS; i++)*/
  /*printf("   BORDER %d,osa:%g :%d\n",i,pow(0.5,i+1),recresult->splitborders[i]);*/
...
int nlayer;
{
  ...
  if(physical->Damage) /* && layers->layer[nlayer].Namorph>1) */
	...
	if(k>=ZBOXES) {*namorph=0;goto awayd;}
	else if(k<0)  {*namorph=0;goto awayd;}
...
int model; 
{
  ...
  else if(model==3){ /*half-gaussian distribution*/
...
real *Ymean;
{
  ...
  if(n==0){  /*if inner loop*/
   ...
   if(physical->Damage) 
     ...
     if(reccalc->reed) FDn2*=pow(0.5,recoil->splitnum);
